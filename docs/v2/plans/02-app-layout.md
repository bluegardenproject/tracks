...
Out of scope: Main's header and tabs (chunk 3), real tracks (chunk 5), storage and hooks.
...
- `TrackView` is plain display data: number, name, kind, status, repo names, window ID, and whether the track needs attention.

## Theme (`internal/v2/ui/theme`)

- The palette values from [PR #105](https://github.com/bluegardenproject/tracks/pull/105), ported to Lip Gloss v2 colours, plus a Huh v2 form theme.
- The same status colours for the footer (as tmux colour names) and for Bubble Tea screens, from one table.
- Library-neutral helpers (control-character stripping, status to colour as plain data) sit in a package without Charm imports, so v1 can share them later if wanted.
...
One fixed row, the tmux status line, on every window, Main included.

```text
 ⌂ Main │ «  ‹ ●1 │ 3 api-auth │ 4 ui-login │ 5 docs-fix │ 6 billing │ 7 infra │ › » │ ▶ 2 dev  ⇄ proxy │ 192.168.1.20 · 85.14.3.9
```

- **Main** is pinned on the left.
...
## Main window, placeholder (`internal/v2/ui/mainview`)
...
- **Full menu (`Alt+p`):**
  ...
  - in the demo, actions act on fake tracks: switch, "end" marks a track finished, "new" opens the form
...
- What Main needs, as input for chunk 3.
