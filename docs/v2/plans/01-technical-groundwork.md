...
- **Packages for chunk 1 (agreed):**
  - `internal/v2/cli`: commands: the root, `--demo`, `stop`, hidden helpers (the fake agent and dev server, the Tracks window, key-binding commands)
  ...
  - `internal/v2/trackwin`: a track's window, its panes and their layout (real code; the demo uses it)
...
- **Generated config** (`conf.go`, `text/template`):
  ...
  - pane titles on the top border, from the `@tracks_title` pane option (programs overwrite tmux's own pane title); window 0 turns them off
  - border and title colours are plain values filled in by `cli` from theme tokens, in the dark or light variant of the terminal that starts the server
  - chunk 2 adds the footer settings
  - contents today (colours and the command shortened):
...
set -g pane-border-format " #{?pane_active,<title.active>,<title>}#{@tracks_title}#[default] "
set -g pane-border-style "fg=<border>"
set -g pane-active-border-style "fg=<border.active>"
bind-key t run-shell -b "<tracks> trackwin add-terminal '#{window_id}'"
...
`--demo` is a flag of the root command, so every command takes it (`paths`, `stop`, the hidden helpers).

- **`./tracks --new-app --demo`:** the same start-up as the real app, on socket `tracks-v2-demo`. When it creates the session it first wipes the demo data directory, and after window 0 it opens the fake track windows, then attaches.
- **`./tracks --new-app --demo stop`** kills the demo server.
- **Real, in `internal/v2/trackwin`:**
...
- **Fake, in `internal/v2/demo`:**
  ...
  - the hidden `demo agent --scenario implementing|question|review` prints scripted, Claude-like output with pauses, then answers typed lines with "This is a demo agent"
  - the hidden `demo devserver` prints a fake request log
- Chunk 2 puts the `Source` interface in front of the fake tracks.
...
- **Integration, on `tmuxtest`:**
  ...
  - a track window: roles, titles, column width, and even heights after adding a terminal
- **Manual:**
